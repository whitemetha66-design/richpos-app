...
def init_db():
    ...
    try:
        cursor.execute("ALTER TABLE inventory ADD COLUMN category TEXT DEFAULT 'ทั่วไป'")
...
if page == "🛒 1. หน้าขาย (POS)":
        ...
        if not filtered_menus:
            ...
            for idx, item in enumerate(filtered_menus):
                with grid_cols[idx % 2]:
                    ...
                    st.image(item["img_url"] if item["img_url"] else "https://via.placeholder.com/200",
                             use_container_width=True)
                    st.markdown(f"### {item['name']}")
                    st.markdown(f'<div class="price-tag">฿{item["price"]:.2f}</div>', unsafe_allow_html=True)

                    sweet_level = st.selectbox("หวาน", ["หวานปกติ (100%)", "หวานน้อย (50%)", "หวาน 25%", "ไม่หวาน (0%)",
                                                        "หวานมาก (125%)"], key=f"sweet_{item['id']}")

                    if st.button(f"🛒 เพิ่มลงตะกร้า", key=f"menu_add_{item['id']}", use_container_width=True,
                                 type="primary"):
    ...
    with col_cart:
        ...
        with c_table:
            table_no = st.selectbox("เลือกโต๊ะ / บริการ",
                                    ["โต๊ะ A1", "โต๊ะ A2", "โต๊ะ B1", "โต๊ะ B2", "กลับบ้าน (Takeaway)"])
        with c_cname:
        ...
        if st.session_state.current_member:
            st.info(
                f"👤 **{st.session_state.current_member['name']}** ({st.session_state.current_member['points']} แต้ม)")
        ...
        if st.button("🚀 ส่งเข้าครัว / ออกบิล ➔", key="btn_submit_order", use_container_width=True, type="primary",
                     disabled=(subtotal == 0)):
            ...
            for cart_key, item_data in st.session_state.cart.items():
                ...
                for r in recipes:
                    ...
                    cursor.execute(
                        "INSERT INTO stock_logs (inventory_id, change_qty, balance_qty, type, note) VALUES (?, ?, ?, ?, ?)",
                        (r["inventory_id"], -used_qty, new_inv["stock"], 'SALE',
                         f"ตัดขาย ออเดอร์ {order_no} ({item_data['sweet']})"))
...
# ==========================================
    ...
    if not pending_orders:
        ...
        with col_list:
            ...
            order_options = {f"{o['order_no']} ({o['table_no']}) - ฿{o['grand_total']:.2f}": o["id"] for o in
                             pending_orders}
            ...
            st.markdown(
                f"**โต๊ะ/บริการ:** {ord_data['table_no']} | **สมาชิก:** {ord_data['member_phone'] or 'ไม่ได้ระบุ'}")
            st.divider()
            for it in items:
                st.write(f"- {it['name']} x{it['qty']} = ฿{it['qty'] * it['price_per_unit']:.2f}")
            st.divider()
            st.markdown(f"<h2 style='color:#4E6C50;'>ยอดชำระสุทธิ: ฿{ord_data['grand_total']:.2f}</h2>",
                        unsafe_allow_html=True)
        ...
        with col_pay:
            ...
            if pay_method == "เงินสด (Cash)":
                cash_received = st.number_input("จำนวนเงินที่รับมา", min_value=float(ord_data['grand_total']),
                                                step=20.0)
                change = cash_received - ord_data['grand_total']
                st.markdown(f"<h3 style='color:#395135;'>เงินทอน: ฿{change:.2f}</h3>", unsafe_allow_html=True)
            else:
                st.image(
                    f"https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=PromptPay_{ord_data['grand_total']}",
                    caption="สแกนเพื่อชำระเงิน")

            if st.button("✅ ยืนยันชำระเงิน & สะสมแต้ม", key="btn_confirm_pay", type="primary",
                         use_container_width=True):
                cursor = conn.cursor()
                cursor.execute("UPDATE orders SET status = 'ชำระเงินแล้ว', payment_method = ? WHERE id = ?",
                               (pay_method, selected_id))

                if ord_data['member_phone']:
                    earned_points = int(ord_data['grand_total'] // 50)
                    cursor.execute("UPDATE members SET points = points + ? WHERE phone = ?",
                                   (earned_points, ord_data['member_phone']))
...
# ==========================================
    ...
    with tab1:
        members = conn.execute(
            "SELECT phone as เบอร์โทร, name as ชื่อลูกค้า, points as แต้มสะสม, created_at as วันที่สมัคร FROM members").fetchall()
        if members:
        ...
        with st.form("add_member_form"):
            ...
            if submit:
                if len(new_phone) >= 9 and new_name:
                    try:
                        conn.execute("INSERT INTO members (phone, name, points) VALUES (?, ?, 0)",
                                     (new_phone, new_name))
                        conn.commit()
...
# ==========================================
    ...
    with tab_dash1:
        ...
        with col_inv:
            ...
            all_cats = ["ทั้งหมด"] + list(
                set([item["category"] if item["category"] else "ทั่วไป" for item in inv_items]))
            selected_filter_cat = st.selectbox("📁 กรองตามหมวดหมู่:", all_cats)

            display_items = inv_items
            if selected_filter_cat != "ทั้งหมด":
                display_items = [item for item in display_items if
                                 (item["category"] or "ทั่วไป") == selected_filter_cat]
    ...
    with tab_dash2:
        ...
        logs = conn.execute("""
                            SELECT l.created_at as วันเวลา, i.name as วัตถุดิบ, l.change_qty as จำนวนที่เปลี่ยน, l.balance_qty as ยอดคงเหลือ, l.type as ประเภท
                                 , l.note       as หมายเหตุ
                            FROM stock_logs l
...
# ==========================================
    ...
    with tab_menu:
        ...
        with st.form("form_add_menu"):
            ...
            m_img = st.text_input("ลิงก์ URL รูปภาพ",
                                  value="https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=500")
        ...
        if menus:
            ...
            with st.form("form_edit_menu"):
                ...
                default_cat_idx = cat_options.index(target_menu["category"]) if target_menu[
                                                                                    "category"] in cat_options else 0
    ...
    with tab_stock:
        ...
        with st.form("form_add_inv_advanced"):
            ...
            inv_cat = col_i2.selectbox("หมวดหมู่วัตถุดิบ",
                                       ["นม/ผลิตภัณฑ์นม", "เมล็ดกาแฟ/ผงชา", "ไซรัป/ซอส", "ท็อปปิ้ง/วัตถุดิบสด",
                                        "บรรจุภัณฑ์ (แก้ว/ฝา/หลอด)", "ของใช้/ทำความสะอาด"])
            ...
            min_stock_alert = col_s2.number_input(f"แจ้งเตือนเมื่อสต็อกต่ำกว่า ({base_unit})", min_value=0.0,
                                                  value=100.0)
            ...
            if st.form_submit_button("💾 บันทึกวัตถุดิบใหม่", type="primary"):
                if inv_name and base_unit:
                    ...
                    cursor.execute("""
                                   ...
                                   VALUES (?, ?, ?, ?, ?, ?, ?)
                                   """,
                                   (inv_name, inv_cat, init_stock_base, base_unit, cost_per_base_unit, min_stock_alert,
                                    supplier))

                    new_id = cursor.lastrowid
                    cursor.execute(
                        "INSERT INTO stock_logs (inventory_id, change_qty, balance_qty, type, note) VALUES (?, ?, ?, ?, ?)",
                        (new_id, init_stock_base, init_stock_base, 'REFILL',
                         f"เพิ่มวัตถุดิบใหม่ ({buy_price} บาท/{buy_unit})"))
                    conn.commit()
        ...
        if invs:
            ...
            selected_inv_label = st.selectbox("เลือกวัตถุดิบที่ต้องการปรับสต็อก", list(inv_dict.keys()),
                                              key="select_inv_stock_op")
        ...
        if invs:
            with st.form("form_edit_inv"):
                ...
                inv_cat_options = ["นม/ผลิตภัณฑ์นม", "เมล็ดกาแฟ/ผงชา", "ไซรัป/ซอส", "ท็อปปิ้ง/วัตถุดิบสด",
                                   "บรรจุภัณฑ์ (แก้ว/ฝา/หลอด)", "ของใช้/ทำความสะอาด", "ทั่วไป"]
                ...
                if st.form_submit_button("💾 บันทึกการแก้ไขวัตถุดิบ", type="primary"):
                    conn.execute(
                        "UPDATE inventory SET name=?, category=?, unit=?, cost_per_unit=?, min_stock=?, supplier=? WHERE id=?",
    ...
    with tab_recipe:
        ...
        if recipes_list:
            rec_dict = {f"{r['menu_name']} ➔ ใช้ {r['inv_name']} ({r['qty_used']} {r['unit']})": r for r in
                        recipes_list}
            selected_rec_label = st.selectbox("เลือกสูตรที่ต้องการจัดการ", list(rec_dict.keys()),
                                              key="select_rec_manage")
            target_rec = rec_dict[selected_rec_label]

            new_recipe_qty = st.number_input("แก้ไขปริมาณที่ใช้ต่อ 1 แก้ว/จาน", value=float(target_rec["qty_used"]),
                                             format="%.3f", key="edit_rec_qty")
    ...
    with tab_mem_edit:
        ...
        if mems:
            ...
            selected_mem_label = st.selectbox("เลือกสมาชิกที่ต้องการจัดการ", list(mem_dict.keys()),
                                              key="edit_mem_select")
